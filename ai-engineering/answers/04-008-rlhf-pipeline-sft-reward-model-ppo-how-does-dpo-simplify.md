...
| Annotation cost | High-quality pairwise comparisons: ~$30–100/prompt | Synthetic pairs from stronger judge (a frontier model) for cheap preference data |
...
- Zephyr-7B (HuggingFace 2023): SFT on UltraChat + DPO on UltraFeedback synthetic preference pairs generated by a frontier model — competitive with a small fast model on MT-Bench at 7B scale
...
- **Mistake:** Saying DPO is strictly better than RLHF — **Better:** Acknowledge that DPO is an offline method; frontier models (a frontier model, Claude) still use RLHF variants because iterative online RM retraining on policy-generated samples captures distribution shift that DPO misses; DPO is best for smaller-scale production alignment where simplicity and stability outweigh the quality ceiling
