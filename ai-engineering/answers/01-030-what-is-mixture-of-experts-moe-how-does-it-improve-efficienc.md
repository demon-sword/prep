...
3. **Load balancing loss:** An auxiliary loss penalizes routing imbalance (one expert getting all tokens). Without it, training collapses to using 1–2 experts. Mixtral and Switch Transformer both use variants of this.
...
|-------|-------------|---------------------|-------|
| Mixtral 8×7B | ~47B | ~13B | 2/8 |
| Switch Transformer | 1.6T | ~26B | 1/256 |
| GPT-4 (reported) | ~1.76T | ~220B | 2/16 |
| Gemini 1.5 Pro | ~1T+ | undisclosed | undisclosed |

**Advantages:**
- Same inference compute as a much smaller dense model, but benchmark performance of a much larger one
- Mixtral 8×7B matches or beats Llama 2 70B on most benchmarks at half the active FLOPs
...
**Opening (30s):**
"Mixture of Experts is the architecture that lets you scale a model's parameter count and knowledge capacity without scaling the compute cost of each forward pass. It's the key insight behind models like Mixtral 8×7B and reportedly GPT-4. I'll explain the mechanism, walk through a concrete example, and cover the tradeoffs — especially memory, which is often the gotcha."
...
So for Mixtral 8×7B: 47 billion total parameters, but only about 13 billion are activated per token. The model gets the knowledge capacity of 47B but pays the compute cost of 13B — roughly matching a 13B dense model in FLOPs. And the benchmarks bear this out: Mixtral matches or beats Llama 2 70B at half the active FLOPs.
