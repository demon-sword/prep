...
### Concept
A production RAG system lives inside a **tradeoff triangle**: latency, retrieval/generation accuracy, and cost. Increasing chunk size improves context completeness (accuracy ↑) but raises token spend and LLM latency (cost ↑, latency ↑). Retrieving more candidates (higher top-k) helps recall but feeds more tokens to the LLM and adds reranking time. Swapping a frontier model for a small fast model cuts cost 10–20× but may reduce answer quality. Every tuning decision moves at least two corners of the triangle simultaneously.
...
| Use larger LLM (frontier vs small tier) | +200–500 ms | +complex reasoning |
...
|---------------|-------------------|----------------------|
| Generation model | a small fast model (~$1/1M in) | a frontier model (~$5/1M in) |
...
- A small fast model + aggressive caching: ~$2,500–3,000/day
- A frontier model + no caching: ~$22,000–25,000/day
- Model tiering (small tier for routing, frontier for escalations): ~$5,500–6,000/day with better quality on hard queries

### Example / Tradeoff

**Real tuning sequence (customer support chatbot):**

1. **Baseline:** chunk=512, top-k=5, no reranker, a small fast model → 68% faithfulness, p95 latency 1.1 s, $0.004/query
...
4. **Add semantic cache (GPTCache, θ=0.93, TTL=1 hr):** 28% hit rate, avg latency → 0.92 s (cache path ~0.1 s), cost → $0.0029/query
5. **Final state:** 84% faithfulness, 0.92 s p95, $0.0029/query — better accuracy and lower cost than baseline
...
- p95 latency ≤ 2.0 s
- Cost per query ≤ $0.005 (customer support SLO)
...
The third axis is **cost vs quality**. The biggest lever is model tiering. A small fast model is roughly 5× cheaper than a frontier model for generation, so I route routine queries to the small tier and escalate hard or low-confidence queries to the frontier tier. A concrete example: at 1M queries/day, frontier-only runs about $22,500/day; tiered routing with caching brings it under $6K/day with comparable end-user quality on the queries that matter."
