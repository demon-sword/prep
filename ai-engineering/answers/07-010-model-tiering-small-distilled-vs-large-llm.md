...
Interviewers want to see whether you understand that blindly routing all traffic to a frontier model (frontier models) is a cost anti-pattern. The question probes your ability to design an intelligent routing layer that preserves quality on complex queries while dramatically reducing cost on simpler ones — a real production challenge at any non-trivial scale.
...
### Concept
Model tiering is a cost-optimization pattern where requests are routed to different LLMs based on task complexity: cheap, fast small models (a small fast model, a small open-weight model (7–8B class)) handle high-volume simple queries; expensive frontier models (frontier models) are reserved for tasks that genuinely require their capability. The key insight is that in most production systems, 60–80% of queries are FAQ-style, lookup, or classification tasks that a 7–8B model handles with >95% accuracy — paying frontier-model prices for these is pure waste.
...
3. **Define model tiers:**
   - **Tier 1 (cheap/fast):** a small fast model ($1/M input, $5/M output), a small open-weight model (7–8B class) self-hosted (~$0.001/M at 4× A10G), or a small open-weight model (7–8B class). Target: FAQ, slot-filling, classification, short summarization.
   - **Tier 2 (mid):** a small fast model with extended context, a 70B-class open-weight model. Target: medium-complexity retrieval + synthesis, most support chatbot turns.
   - **Tier 3 (frontier):** a frontier model ($5/M input, $25/M output), a frontier model. Target: multi-step reasoning, long-context synthesis, code generation, regulated-domain queries.
...
- Profiling shows 65% of queries are plan lookups, account status, or FAQ — answerable by a 8B model with 96% accuracy vs a frontier model's 98% (acceptable 2-point gap for this use case).
- Router: DistilBERT classifier trained on 2K labeled complexity examples, p99 latency 4ms.
- Tier assignment: 65% → a small fast model ($1/M), 30% → a small fast model + extended context, 5% → a frontier model ($5/M).
- **Cost math:**
  - Naive all-frontier: 500K × 800 avg tokens × $5/M ≈ **$2,000/day**
  - Tiered: 95% × $1 rate + 5% × $5 rate ≈ **$480/day** (76% reduction)
- **Tradeoff:** Router adds ~5ms latency; the 5% that escalates to a frontier model adds ~$100/day but protects quality on hard queries. If the router misfires (routes a hard query to Tier-1), you get a quality hit — so golden-set validation before deploying the router is mandatory.
- **Self-hosted angle:** Self-hosting is no longer primarily a cost play — hosted small-tier models are cheap enough that the cheap tier is a small share of a blended bill. Justify it on data residency, latency control, or a fine-tuned task-specific model, and measure your own GPU economics rather than quoting a published break-even figure.
...
The tiers I'd define are: Tier 1 for cheap and fast tasks — a small fast model at $1/M tokens or a self-hosted a small open-weight model (7–8B class); Tier 2 for mid-complexity; and Tier 3 reserving a frontier model for the hardest 5–10% of queries. Before deploying the router, I'd benchmark each tier on a golden evaluation set — if Tier-1 is within 3 percentage points of Tier-3 quality on the SLO metric, that gap is usually acceptable. The cost math for a 500K-query/day system goes from about $2,000/day all-frontier down to roughly $480/day with tiering — about a 76% reduction.

A concrete example: a customer support chatbot where 65% of queries are plan lookups and account status. The 8B model answers those at 96% accuracy vs 98% for a frontier model — that 2-point gap is acceptable. The 5% of complex billing disputes or multi-turn investigations go to a frontier model. The router itself runs a DistilBERT classifier in 4ms."

**Tradeoff / production angle (1 min):**
"The main risk is router miscalibration — if a complex query is mis-routed to Tier-1, you get a quality regression. So I'd add a fallback: monitor structured-output parse failures or low-confidence signals from Tier-1 and escalate to Tier-2 automatically. I'd also track per-tier thumbs-down rate in production and refresh the router's decision threshold monthly. The other risk is self-hosting economics — you need to validate that GPU provisioning costs are lower than API spend before committing — and with hosted small-tier models as cheap as they now are, that bar is higher than it used to be, so measure it for your own workload rather than quoting a break-even figure."
...
> Route 60–80% of low-complexity queries to a cheap fast model (a small fast model, $1/M) via a lightweight complexity classifier, reserve the frontier model for the hard 5–10%, validate routing accuracy on a golden set, and monitor per-tier quality drift — typically yields 70–80% cost reduction on routed traffic.
