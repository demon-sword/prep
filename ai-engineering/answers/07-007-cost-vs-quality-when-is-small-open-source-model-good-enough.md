...
The interviewer is testing whether you understand that model selection is an engineering tradeoff, not a default to "the best model." Strong candidates can articulate a structured decision process — task complexity, quality SLOs, latency budget, cost at scale, and data-privacy requirements — rather than defaulting to a frontier model or assuming bigger is always better.

### Trigger phrases
- "When would you use a smaller open-source model instead of a frontier model?"
...
| Grounded RAG summarization (closed-world) | Medium | 7–13B or a small fast model |
...
- Run the candidate small model and a frontier model (or the frontier baseline) on identical prompts
...
|-------|-----------|------------|-----------|
| A frontier model (1K in / 200 out tokens avg) | $5/1M | $25/1M | ~$10,000 |
| A small fast model (hosted) | $1/1M | $5/1M | ~$2,000 |
| A small open-weight model (7–8B class), self-hosted | GPU cost, not per-token | fixed | measure your own GPU economics |
| A small open-weight model (7–8B class) on a third-party host | per-token, varies by host | | not verified — check current host pricing |

If the smaller model achieves acceptable quality, the hosted small tier is ~5× cheaper, and a self-hosted fine-tuned model can go further — but that gap is narrower than it was in 2024, so measure it rather than assuming an order of magnitude. Fine-tuning a 7B model on domain data with QLoRA typically costs $20–100 one-time and often matches a frontier model's task accuracy.
...
### Example / Tradeoff
**Customer support intent router:** Replace a frontier model with a fine-tuned a small open-weight model (7–8B class) (QLoRA, 5K labeled examples). Benchmark result: 96.4% F1 vs a frontier model's 97.1% — Δ0.7%, well within SLO. Cost drops from ~$8K/day to ~$250/day (32× reduction). Latency drops from ~1.8s p95 to ~0.4s because the model runs on-prem with vLLM.

**Where small models fail:** Open-domain factual QA, complex multi-hop reasoning, and creative writing — tasks that genuinely require the breadth of frontier pretraining. Forcing a small open-weight model (7–8B class) on legal contract analysis without fine-tuning can drop accuracy 15–20 points.
...
Third, I do the cost math. At 1M queries per day, a frontier model costs about $10,000/day at typical token lengths. The hosted small tier drops that to around $2,000/day — a 5× gap, not the order of magnitude it used to be. A self-hosted a small open-weight model (7–8B class) on four A100s runs about $200/day fixed, or I can use Fireworks or Together AI for around $120–240 variable. Fine-tuning the 7B model on domain data with QLoRA costs $20–100 one-time and often closes the remaining accuracy gap.

A concrete example: I replaced a frontier model as an intent router for a customer support system with a QLoRA fine-tuned a small open-weight model (7–8B class). The benchmark showed 96.4% vs 97.1% F1 — a 0.7-point delta that was within our SLO. Cost went from $8K/day to $250/day, and p95 latency dropped from 1.8s to 0.4s because the model ran on-prem with vLLM."

**Tradeoff / production angle (1 min):**
"The failure mode is over-indexing on benchmarks. A 7B model that scores 88% on MMLU is not interchangeable with a frontier model on MMLU-level tasks in your domain. Academic benchmarks are saturated and contaminated. The only reliable signal is a task-specific golden dataset built from your actual production queries. Also, small models degrade faster on complex multi-step reasoning — if the task requires synthesizing across six documents or doing multi-hop inference, the accuracy delta grows quickly and you're better off with a larger model."

**Wrap-up (30s):**
"The bottom line: for low-to-medium complexity tasks with a stable domain, a fine-tuned small open-source model is almost always good enough and meaningfully cheaper. The decision gate is always a task-specific benchmark on production data, not a gut feeling about model size."

---

## Pitfalls

- **Mistake:** Defaulting to a frontier model for everything "to be safe" without benchmarking — **Better:** Build a 100-sample golden dataset and measure the actual accuracy delta; quantify the cost difference and present it as a justified engineering decision.
