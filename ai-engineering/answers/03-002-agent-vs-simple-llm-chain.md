...
"The failure mode I see most is over-indexing on agents. Teams reach for an agent because they want flexibility, but the task graph was actually enumerable — they just didn't model it. The result is variable cost, flaky tests, and debugging nightmares. My default is: if I can draw the flowchart at design time, I use a chain or a LangGraph DAG. I upgrade to a ReAct agent only when the step count or branching is genuinely unknown at design time. And when I do use an agent, I always add a `max_iterations` cap, model-tiered routing (mini for tool selection, full model for synthesis), and LangSmith traces from day one."
