...
TOOL_NAMES = [n.strip() for n in """vLLM|TensorRT|Triton|PyTorch|TensorFlow|Hugging Face|Transformers|LangChain|
LlamaIndex|Ray|DeepSpeed|Megatron|FlashAttention|Flash Attention|PagedAttention|LoRA|QLoRA|
bitsandbytes|PEFT|TRL|Llama|Mistral|Qwen|DeepSeek|GPT|Claude|Gemini|OpenAI|Anthropic|Pinecone|
Weaviate|Qdrant|Chroma|FAISS|Milvus|Elasticsearch|MLflow|Ollama|llama.cpp|GGUF|ONNX|A100|H100|
TPU|CUDA|NCCL|SGLang|TGI|Axolotl|Unsloth|SentencePiece|tiktoken|Weights & Biases|W&B|LangSmith|
Langfuse|Braintrust|Outlines|xFormers|ZeRO|FSDP|Accelerate|Gradio|Streamlit|NVIDIA|Bedrock|
Vertex AI|SageMaker|Together|Fireworks|Groq|Modal|BentoML|Guidance|DSPy|Instructor|Pydantic""".replace("\n", "").split("|") if n.strip()]
