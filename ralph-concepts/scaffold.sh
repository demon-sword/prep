...
# Usage: ./ralph-concepts/scaffold.sh
# Builds ralph-concepts/plan.md from concepts.json.
...
if plan_file.exists():
    for line in plan_file.read_text().splitlines():
        if line.strip().startswith("- [x]"):
            # Extract id from line like "- [x] 01-self-supervision — ..."
...
plan = f"""# Plan: AI Engineering Concepts
...
print(f"Scaffolded ralph-concepts plan")
...
if remaining > 0:
    print("Next:")
    print(f"  ./ralph-concepts/once.sh")
    print(f"  ./ralph-concepts/loop.sh {remaining + 5}")
else:
