...
# Usage: ./ralph-mlops/loop.sh [concept-id] [max_iterations] [--model <slug>]
#
...
#
# Examples:
#   ./ralph-mlops/loop.sh                        # run all remaining (default max=50)
#   ./ralph-mlops/loop.sh 40                     # run all remaining, max 40 iterations
#   ./ralph-mlops/loop.sh 04-feature-stores       # retry one specific concept
#   ./ralph-mlops/loop.sh 19-drift-types --model claude-sonnet-4
...
if [[ -n "$TARGET_CONCEPT" ]]; then
  echo "Starting Ralph MLOps loop — concept: $TARGET_CONCEPT (max $MAX retries)"
else
  echo "Starting Ralph MLOps loop — all remaining (max $MAX iterations)"
fi
echo "Concepts dir: $SCRIPT_DIR/../mlops/concepts/"
echo ""
...
for ((i = 1; i <= MAX; i++)); do
  ...
  if [[ "$RC" -eq 0 ]]; then
    ...
    if [[ -z "$TARGET_CONCEPT" ]]; then
      ...
      DRC=0
      ralph_validate bash "$PREP_ROOT/validate-corpus.sh" --final "mlops" || DRC=$?
      if [[ "$DRC" -ne 0 ]]; then
...
echo ""
echo "Reached max iterations ($MAX). Check ralph-mlops/plan.md."
set +e
bash "$SCRIPT_DIR/validate.sh"
VRC=$?
set -e
