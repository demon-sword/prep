...
# Usage: ./ralph-mlops/once.sh [concept-id] [--model <slug>]
#
# concept-id (optional): generate a specific concept by id, e.g. 04-feature-stores
#                        If omitted, picks the first unchecked item in plan.md.
#
# Examples:
#   ./ralph-mlops/once.sh                        # next in queue
#   ./ralph-mlops/once.sh 04-feature-stores       # specific concept
#   ./ralph-mlops/once.sh 19-drift-types --model claude-sonnet-4
...
MODEL="${RALPH_MLOPS_MODEL:-}"
AGENT_TIMEOUT_SEC="${RALPH_MLOPS_TIMEOUT:-3600}"
TARGET_CONCEPT=""
...
OUTPUT_DIR="$PREP_ROOT/mlops/concepts"
CORPUS_TRACK="mlops"
OUTPUT_EXT="html"
...
[[ -f "$PLAN" ]] || {
  echo "Missing $PLAN — run ./ralph-mlops/scaffold.sh first" >&2
  exit 1
}
...
" >&2
  ...
  HTML_PATH="$PREP_ROOT/mlops/concepts/${TARGET_CONCEPT}.html"
  if [[ -f "$HTML_PATH" ]]; then
...
The file should be at: $PREP_ROOT/mlops/concepts/${TARGET_CONCEPT}.html
Mark it as done in ralph-mlops/plan.md even if it was already checked."
else
  DISPLAY_CONCEPT="(next in queue)"
  TARGET_INSTRUCTION="Pick the FIRST unchecked item from the ## Next section of ralph-mlops/plan.md."
fi

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Ralph MLOps — concept: $DISPLAY_CONCEPT"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Log: $LOG_FILE" >&2
...
for ((attempt = 1; attempt <= RALPH_MAX_ATTEMPTS; attempt++)); do
  ...
  TASK="You are generating interactive HTML concept files for MLOps / production-ML interview study.

WORKSPACE ROOT: $PREP_ROOT
OUTPUT DIR: $PREP_ROOT/mlops/concepts/
...
After you finish, ralph runs two gates you cannot bypass:
  1. bash ralph-mlops/validate.sh <id> — a hard gate. Its exit code is honoured.
...
${FEEDBACK}"
  ...
  if [[ "$VALIDATE_RC" -eq 0 && -n "$RESOLVED_CONCEPT" ]]; then
    FACT_RC=0
    ralph_factcheck "$PREP_ROOT/mlops/concepts/${RESOLVED_CONCEPT}.html" "an MLOps / production-ML study page" "$MODEL" || FACT_RC=$?
