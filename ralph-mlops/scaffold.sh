...
# Usage: ./ralph-mlops/scaffold.sh
# Builds ralph-mlops/plan.md from concepts.json.
...
CONCEPTS_DIR="$PREP_ROOT/mlops/concepts"
...
if plan_file.exists():
    for line in plan_file.read_text().splitlines():
        if line.strip().startswith("- [x]"):
            # Extract id from line like "- [x] 01-data-validation — ..."
...
print(f"Scaffolded ralph-mlops plan")
...
if remaining > 0:
    print("Next:")
    print(f"  ./ralph-mlops/once.sh")
    print(f"  ./ralph-mlops/loop.sh {remaining + 5}")
else:
