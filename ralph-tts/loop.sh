...
# Usage: ./ralph-tts/loop.sh [concept-id] [max_iterations] [--model <slug>]
#
...
#
# Examples:
#   ./ralph-tts/loop.sh                        # run all remaining (default max=50)
#   ./ralph-tts/loop.sh 40                     # run all remaining, max 40 iterations
#   ./ralph-tts/loop.sh 04-feature-stores       # retry one specific concept
#   ./ralph-tts/loop.sh 19-drift-types --model claude-sonnet-4
...
if [[ -n "$TARGET_CONCEPT" ]]; then
  echo "Starting Ralph TTS loop — concept: $TARGET_CONCEPT (max $MAX retries)"
else
  echo "Starting Ralph TTS loop — all remaining (max $MAX iterations)"
fi
echo "Concepts dir: $SCRIPT_DIR/../tts/concepts/"
echo ""
...
for ((i = 1; i <= MAX; i++)); do
  ...
  if [[ "$RC" -eq 0 ]]; then
    ...
    if [[ -z "$TARGET_CONCEPT" ]]; then
      ...
      DRC=0
      ralph_validate bash "$PREP_ROOT/validate-corpus.sh" --final "$CORPUS_TRACK" || DRC=$?
      if [[ "$DRC" -ne 0 ]]; then
...
echo ""
echo "Reached max iterations ($MAX). Check ralph-tts/plan.md."
set +e
bash "$SCRIPT_DIR/validate.sh"
VRC=$?
set -e
