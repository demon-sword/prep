...
# Usage: ./ralph-tts/once.sh [concept-id] [--model <slug>]
#
...
#
# Examples:
#   ./ralph-tts/once.sh                        # next in queue
#   ./ralph-tts/once.sh 04-feature-stores       # specific concept
#   ./ralph-tts/once.sh 19-drift-types --model claude-sonnet-4
...
MODEL="${RALPH_TTS_MODEL:-}"
AGENT_TIMEOUT_SEC="${RALPH_TTS_TIMEOUT:-3600}"
TARGET_CONCEPT=""
...
OUTPUT_DIR="$PREP_ROOT/tts/concepts"
CORPUS_TRACK="tts"
OUTPUT_EXT="html"
...
[[ -f "$PLAN" ]] || {
  echo "Missing $PLAN — run ./ralph-tts/scaffold.sh first" >&2
  exit 1
}
...
" >&2
  ...
  HTML_PATH="$PREP_ROOT/tts/concepts/${TARGET_CONCEPT}.html"
  if [[ -f "$HTML_PATH" ]]; then
...
The file should be at: $PREP_ROOT/tts/concepts/${TARGET_CONCEPT}.html
Mark it as done in ralph-tts/plan.md even if it was already checked."
else
  DISPLAY_CONCEPT="(next in queue)"
  TARGET_INSTRUCTION="Pick the FIRST unchecked item from the ## Next section of ralph-tts/plan.md."
fi

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Ralph TTS — concept: $DISPLAY_CONCEPT"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Log: $LOG_FILE" >&2
...
for ((attempt = 1; attempt <= RALPH_MAX_ATTEMPTS; attempt++)); do
  ...
  TASK="You are generating interactive HTML concept files for text-to-speech and voice-AI interview study.

WORKSPACE ROOT: $PREP_ROOT
OUTPUT DIR: $PREP_ROOT/tts/concepts/
...
After you finish, ralph runs two gates you cannot bypass:
  1. bash ralph-tts/validate.sh <id> — a hard gate. Its exit code is honoured.
...
${FEEDBACK}"
  ...
  if [[ "$VALIDATE_RC" -eq 0 && -n "$RESOLVED_CONCEPT" ]]; then
    FACT_RC=0
    ralph_factcheck "$PREP_ROOT/tts/concepts/${RESOLVED_CONCEPT}.html" "a text-to-speech / voice-AI study page" "$MODEL" || FACT_RC=$?
