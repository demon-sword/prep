...
# Usage: ./ralph-ai-engineering/loop.sh <category-slug> [max_iterations] [--agent cursor|claude] [--model <slug>]
# Example: ./ralph-ai-engineering/loop.sh 02-rag-systems 40 --agent claude
#
# Runs once.sh until COMPLETE promise or max iterations.
# Default MAX=80 — handles 10-behavioral (74 questions = 76 tasks) with margin.
...
LOOP_WATCH=("$PREP_ROOT/ai-engineering/answers" "$PREP_ROOT/ai-engineering/categories")
...
echo "Starting Ralph AI Engineering loop for $CATEGORY (max $MAX iterations, agent: $AGENT_BACKEND)"
...
for ((i = 1; i <= MAX; i++)); do
  ...
  if [[ "$rc" -eq 0 ]]; then
    ...
    CRC=0
    ralph_require_complete "$PREP_ROOT/ai-engineering/plan.md" "$PREP_ROOT/ai-engineering/answers" md "$PREP_ROOT/ai-engineering/categories" || CRC=$?
    if [[ "$CRC" -ne 0 ]]; then
    ...
    echo ""
    echo "Next category: run ./ralph-ai-engineering/scaffold.sh <next-slug> then loop again."
    exit 0
  fi
...
echo "Reached max iterations ($MAX). Check ai-engineering/plan.md and ai-engineering/progress.txt."
set +e
