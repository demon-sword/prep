...
# Usage: ./ralph-machine-learning/once.sh [concept-id] [--model <slug>]
#
# concept-id (optional): generate a specific concept by id, e.g. 14-gradient-boosting
#                        If omitted, picks the first unchecked item in plan.md.
#
# Examples:
#   ./ralph-machine-learning/once.sh                          # next in queue
#   ./ralph-machine-learning/once.sh 14-gradient-boosting     # specific concept
#   ./ralph-machine-learning/once.sh 19-pca --model claude-sonnet-4
...
MODEL="${RALPH_ML_MODEL:-}"
AGENT_TIMEOUT_SEC="${RALPH_ML_TIMEOUT:-3600}"
TARGET_CONCEPT=""
...
OUTPUT_DIR="$PREP_ROOT/machine_learning/concepts"
CORPUS_TRACK="machine-learning"
OUTPUT_EXT="html"
...
[[ -f "$PLAN" ]] || {
  echo "Missing $PLAN — run ./ralph-machine-learning/scaffold.sh first" >&2
  exit 1
}
...
" >&2
  ...
  HTML_PATH="$PREP_ROOT/machine_learning/concepts/${TARGET_CONCEPT}.html"
  if [[ -f "$HTML_PATH" ]]; then
...
The file should be at: $PREP_ROOT/machine_learning/concepts/${TARGET_CONCEPT}.html
Mark it as done in ralph-machine-learning/plan.md even if it was already checked."
else
  DISPLAY_CONCEPT="(next in queue)"
  TARGET_INSTRUCTION="Pick the FIRST unchecked item from the ## Next section of ralph-machine-learning/plan.md."
fi

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Ralph Machine Learning — concept: $DISPLAY_CONCEPT"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Log: $LOG_FILE" >&2
...
for ((attempt = 1; attempt <= RALPH_MAX_ATTEMPTS; attempt++)); do
  ...
  TASK="You are generating interactive HTML concept files for Machine Learning interview study.

WORKSPACE ROOT: $PREP_ROOT
OUTPUT DIR: $PREP_ROOT/machine_learning/concepts/
...
After you finish, ralph runs two gates you cannot bypass:
  1. bash ralph-machine-learning/validate.sh <id> — a hard gate. Its exit code is honoured.
...
${FEEDBACK}"
  ...
  if [[ "$VALIDATE_RC" -eq 0 && -n "$RESOLVED_CONCEPT" ]]; then
    FACT_RC=0
    ralph_factcheck "$PREP_ROOT/machine_learning/concepts/${RESOLVED_CONCEPT}.html" "a machine-learning study page" "$MODEL" || FACT_RC=$?
