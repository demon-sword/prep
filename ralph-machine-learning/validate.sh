...
# Usage: ./ralph-machine-learning/validate.sh [concept-id]
# Without argument: validates all generated concepts.
# With argument: validates one concept (e.g. 14-gradient-boosting).
#
...
TOOL_NAMES = [n.strip() for n in """scikit-learn|sklearn|XGBoost|statsmodels|LightGBM|CatBoost|PyTorch|
TensorFlow|Keras|NumPy|pandas|SciPy|JAX|Optuna|Hyperopt|SHAP|LIME|imbalanced-learn|SMOTE|Prophet|
MLlib|cuML|FAISS|Annoy|HNSW|UMAP|HDBSCAN|Matplotlib|Seaborn|Plotly|Weights & Biases|W&B|MLflow|
Spark|Dask|Numba|Cython|Ray|PyMC|Stan|Gensim|spaCy|NLTK|Optimizely|Statsig|Eppo|GrowthBook|
Vowpal Wabbit|H2O|Surprise|Featuretools|TPOT|Yellowbrick|Bayesian Optimization|Scikit-Optimize""".replace("\n", "").split("|") if n.strip()]
...
# Report
print(f"\nValidation report — ralph-machine-learning")
