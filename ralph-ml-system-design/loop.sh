...
# Usage: ./ralph-ml-system-design/loop.sh [concept-id] [max_iterations] [--model <slug>]
#
...
#
# Examples:
#   ./ralph-ml-system-design/loop.sh                        # run all remaining (default max=50)
#   ./ralph-ml-system-design/loop.sh 40                     # run all remaining, max 40 iterations
#   ./ralph-ml-system-design/loop.sh 01-feed-ranking-system       # retry one specific concept
#   ./ralph-ml-system-design/loop.sh 03-feature-store --model claude-sonnet-4
...
if [[ -n "$TARGET_CONCEPT" ]]; then
  echo "Starting Ralph ML System Design loop — concept: $TARGET_CONCEPT (max $MAX retries)"
else
  echo "Starting Ralph ML System Design loop — all remaining (max $MAX iterations)"
fi
echo "Concepts dir: $SCRIPT_DIR/../ml-system-design/"
echo ""
...
for ((i = 1; i <= MAX; i++)); do
  ...
  if [[ "$RC" -eq 0 ]]; then
    ...
    if [[ -z "$TARGET_CONCEPT" ]]; then
      ...
      DRC=0
      ralph_validate bash "$PREP_ROOT/validate-corpus.sh" --final "ml-system-design" || DRC=$?
      if [[ "$DRC" -ne 0 ]]; then
...
echo ""
echo "Reached max iterations ($MAX). Check ralph-ml-system-design/plan.md."
set +e
bash "$SCRIPT_DIR/validate.sh"
VRC=$?
set -e
