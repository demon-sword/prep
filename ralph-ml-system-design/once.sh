...
# Usage: ./ralph-ml-system-design/once.sh [concept-id] [--model <slug>]
#
# concept-id (optional): generate a specific concept by id, e.g. 01-feed-ranking-system
#                        If omitted, picks the first unchecked item in plan.md.
#
# Examples:
#   ./ralph-ml-system-design/once.sh                        # next in queue
#   ./ralph-ml-system-design/once.sh 01-feed-ranking-system       # specific concept
#   ./ralph-ml-system-design/once.sh 03-feature-store --model claude-sonnet-4
...
MODEL="${RALPH_MLSD_MODEL:-}"
AGENT_TIMEOUT_SEC="${RALPH_MLSD_TIMEOUT:-3600}"
TARGET_CONCEPT=""
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      ...
      python3 -c "
import json, pathlib
concepts = json.loads(pathlib.Path('$SCRIPT_DIR/topics.json').read_text())
for c in concepts:
...
OUTPUT_DIR="$PREP_ROOT/ml-system-design"
CORPUS_TRACK="ml-system-design"
OUTPUT_EXT="html"
...
[[ -f "$PLAN" ]] || {
  echo "Missing $PLAN — run ./ralph-ml-system-design/scaffold.sh first" >&2
  exit 1
}
...
" >&2
  ...
  HTML_PATH="$PREP_ROOT/ml-system-design/${TARGET_CONCEPT}.html"
  if [[ -f "$HTML_PATH" ]]; then
...
The file should be at: $PREP_ROOT/ml-system-design/${TARGET_CONCEPT}.html
Mark it as done in ralph-ml-system-design/plan.md even if it was already checked."
else
  DISPLAY_CONCEPT="(next in queue)"
  TARGET_INSTRUCTION="Pick the FIRST unchecked item from the ## Next section of ralph-ml-system-design/plan.md."
fi

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Ralph ML System Design — concept: $DISPLAY_CONCEPT"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Log: $LOG_FILE" >&2
...
for ((attempt = 1; attempt <= RALPH_MAX_ATTEMPTS; attempt++)); do
  ...
  TASK="You are generating interactive HTML case studies for ML system design interviews.

WORKSPACE ROOT: $PREP_ROOT
OUTPUT DIR: $PREP_ROOT/ml-system-design/
...
After you finish, ralph runs two gates you cannot bypass:
  1. bash ralph-ml-system-design/validate.sh <id> — a hard gate. Its exit code is honoured.
...
${FEEDBACK}"
  ...
  if [[ "$VALIDATE_RC" -eq 0 && -n "$RESOLVED_CONCEPT" ]]; then
    FACT_RC=0
    ralph_factcheck "$PREP_ROOT/ml-system-design/${RESOLVED_CONCEPT}.html" "an ML system-design case study" "$MODEL" || FACT_RC=$?
