...
# Usage: ./ralph-ml-system-design/scaffold.sh
# Builds ralph-ml-system-design/plan.md from topics.json.
...
if plan_file.exists():
    for line in plan_file.read_text().splitlines():
        if line.strip().startswith("- [x]"):
            # Extract id from line like "- [x] 01-feed-ranking-system — ..."
...
print(f"Scaffolded ralph-ml-system-design plan")
...
if remaining > 0:
    print("Next:")
    print(f"  ./ralph-ml-system-design/once.sh")
    print(f"  ./ralph-ml-system-design/loop.sh {remaining + 5}")
else:
