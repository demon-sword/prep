...
# Usage: ./ralph-ml-coding/scaffold.sh
# Builds ralph-ml-coding/plan.md from problems.json.
...
mkdir -p "$PROBLEMS_DIR" "$SCRIPT_DIR/.logs"
touch "$PROGRESS"

python3 - "$PROBLEMS_JSON" "$PLAN" "$PROGRESS" "$PROBLEMS_DIR" <<'PY'
import json
import sys
...
if plan_file.exists():
    for line in plan_file.read_text().splitlines():
        if line.strip().startswith("- [x]"):
            # Extract id from line like "- [x] 30-cosine-topk — ..."
...
plan = f"""# Plan: ML Coding Problems
...
print(f"Scaffolded ralph-ml-coding plan")
...
if remaining > 0:
    print("Next:")
    print(f"  ./ralph-ml-coding/once.sh")
    print(f"  ./ralph-ml-coding/loop.sh {remaining + 5}")
else:
