...
After you finish, ralph runs two gates you cannot bypass:
  1. bash ralph-backend/validate.sh <id> — a hard gate. Its exit code is honoured.
...
${FEEDBACK}"
  ...
  if [[ "$VALIDATE_RC" -eq 0 && -n "$RESOLVED_CONCEPT" ]]; then
    FACT_RC=0
    ralph_factcheck "$PREP_ROOT/backend/concepts/${RESOLVED_CONCEPT}.html" "a backend / distributed-systems study page" "$MODEL" || FACT_RC=$?
