...
# Usage: ./ralph-backend/loop.sh [concept-id] [max_iterations] [--model <slug>]
#
...
#
# Examples:
#   ./ralph-backend/loop.sh                             # run all remaining (default max=50)
#   ./ralph-backend/loop.sh 27                          # run all remaining, max 27 iterations
#   ./ralph-backend/loop.sh 11-consensus-algorithms     # retry one specific concept
#   ./ralph-backend/loop.sh 19-indexing-storage-engines --model claude-sonnet-4

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PREP_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
PLAN="$SCRIPT_DIR/plan.md"
OUTPUT_DIR="$PREP_ROOT/backend/concepts"
OUTPUT_EXT="html"
...
if [[ -n "$TARGET_CONCEPT" ]]; then
  echo "Starting Ralph Backend loop — concept: $TARGET_CONCEPT (max $MAX retries)"
else
  echo "Starting Ralph Backend loop — all remaining (max $MAX iterations)"
fi
echo "Concepts dir: $SCRIPT_DIR/../backend/concepts/"
echo ""
...
for ((i = 1; i <= MAX; i++)); do
  ...
  if [[ "$RC" -eq 0 ]]; then
    ...
    if [[ -z "$TARGET_CONCEPT" ]]; then
      ...
      DRC=0
      ralph_validate bash "$PREP_ROOT/validate-corpus.sh" --final "backend" || DRC=$?
      if [[ "$DRC" -ne 0 ]]; then
...
echo ""
echo "Reached max iterations ($MAX). Check ralph-backend/plan.md."
set +e
bash "$SCRIPT_DIR/validate.sh"
VRC=$?
set -e
