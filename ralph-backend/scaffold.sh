...
# Usage: ./ralph-backend/scaffold.sh
# Builds ralph-backend/plan.md from concepts.json.
...
if plan_file.exists():
    for line in plan_file.read_text().splitlines():
        if line.strip().startswith("- [x]"):
            # Extract id from line like "- [x] 01-caching-strategies — ..."
...
print(f"Scaffolded ralph-backend plan")
...
if remaining > 0:
    print("Next:")
    print(f"  ./ralph-backend/once.sh")
    print(f"  ./ralph-backend/loop.sh {remaining + 5}")
else:
