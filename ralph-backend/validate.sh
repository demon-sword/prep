...
# Usage: ./ralph-backend/validate.sh [concept-id]
# Without argument: validates all generated concepts.
# With argument: validates one concept (e.g. 11-consensus-algorithms).
#
...
CONCEPTS_DIR="$PREP_ROOT/backend/concepts"
CONCEPTS_JSON="$SCRIPT_DIR/concepts.json"

TARGET="${1:-all}"

python3 - "$TARGET" "$CONCEPTS_DIR" "$CONCEPTS_JSON" "$PREP_ROOT" <<'PY'
import json
import re
import sys
import urllib.parse
...
target, concepts_dir_str, concepts_json_path, prep_root = sys.argv[1:5]
...
TOOL_NAMES = [n.strip() for n in """Redis|Kafka|Debezium|Raft|PgBouncer|Postgres|PostgreSQL|MySQL|CockroachDB|
ZooKeeper|etcd|Cassandra|DynamoDB|MongoDB|RabbitMQ|SQS|SNS|Kinesis|Envoy|NGINX|HAProxy|Consul|
Vitess|Citus|ClickHouse|Elasticsearch|Memcached|S3|gRPC|GraphQL|Kubernetes|Docker|Prometheus|
Grafana|Jaeger|Temporal|Flink|Spark|Pulsar|Vault|Istio|TiDB|Spanner|Aurora|MariaDB|SQLite|
RocksDB|LevelDB|HBase|Neo4j|Paxos|Sidekiq|Celery|Varnish|Traefik|Linkerd|OpenTelemetry|Datadog|
Airflow|Maxwell|Hazelcast|Riak|Couchbase|InfluxDB|TimescaleDB|Snowflake|BigQuery|Nomad|Zipkin|
Resilience4j|Hystrix|Finagle|Sentinel|Kong|Cloudflare|Fastly|CloudFront|Akamai""".replace("\n", "").split("|") if n.strip()]
...
SOURCE_TERM_RE = re.compile(r'\*\*([^*\n]{2,60}?)\*\*(?=\s*(?:[—–:(]|$))', re.MULTILINE)


def source_terms(markdown):
    terms = []
    for raw in SOURCE_TERM_RE.findall(markdown):
...
def loose(text):
    """Lowercase with runs of non-alphanumerics collapsed to one space."""
    return re.sub(r'[^a-z0-9]+', ' ', text.lower())


def term_present(term, body):
    """`body` is loose() page text. 'Write-behind (write-back)' is satisfied by
    either the base term or its parenthetical alias."""
    alias = re.match(r'^(.*?)\s*\((.+)\)$', term)
    alternatives = [alias.group(1), alias.group(2)] if alias else [term]
    return any(loose(a).strip() and loose(a).strip() in body for a in alternatives)
...
for c in concepts:
    ...
    # Source fidelity — spec.md promises the theory doesn't drop a named
    # failure mode or pattern from the hand-written source note.
    if c.get("source"):
        source_path = prep_root / c["source"]
        if not source_path.exists():
            concept_errors.append(f"source file missing: {c['source']}")
        else:
            body = loose(strip_tags(html))
            dropped = [t for t in source_terms(source_path.read_text(encoding="utf-8"))
                       if not term_present(t, body)]
            if dropped:
                concept_errors.append(
                    f"named terms from {c['source']} missing from the page: {dropped}"
                )
...
# Report
print(f"\nValidation report — ralph-backend")
...
# Depth queue. `gaps` entries are audit findings against a page that already
# exists; a run that covers them removes the array. Reported, never failed:
# whether a gap is genuinely covered is a judgement no regex can make, and a
# gate that guesses would reject good pages. Counting it keeps the queue from
# going invisible, which is what happened for the six days it sat unread.
pending_gaps = [
    (c["id"], len(c["gaps"]))
    for c in concepts
    if c.get("gaps") and (concepts_dir / f"{c['id']}.html").exists()
]
if pending_gaps:
    total = sum(n for _, n in pending_gaps)
    print()
    print(f"  DEPTH QUEUE: {len(pending_gaps)} generated page(s) carry "
          f"{total} unaddressed gap(s) — advisory, not a failure:")
    for cid, n in pending_gaps:
        print(f"    - {cid} ({n})")
