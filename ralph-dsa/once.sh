...
build_cursor_cmd() {
  ...
  local prompt="@${SPEC} @${README} @${LIST} @${RECOGNITION} @${PROB_TEMPLATE} @${PAT_TEMPLATE} @${PLAN} @${PROGRESS} @${PROMPT_FILE} ${TASK_INSTRUCTIONS}${FEEDBACK}"
...
for ((i = 1; i <= ITERATIONS; i++)); do
  ...
  PRODUCED="$(ralph_newest_since "$ITER_MARKER" "$WORKSPACE/dsa/problems" "$WORKSPACE/dsa/patterns")"
  rm -f "$ITER_MARKER"
  FACT_NOTES=""
  if [[ -n "$PRODUCED" ]]; then
    FACT_RC=0
    ralph_factcheck "$PRODUCED" "a DSA problem or pattern note" "$MODEL" || FACT_RC=$?
  ...
  if has_completion_promise "$promise_text"; then
    ...
    CORPUS_RC=0
    ralph_require_complete "$PLAN" "$WORKSPACE/dsa/problems" md "$WORKSPACE/dsa/patterns" || CORPUS_RC=$?
